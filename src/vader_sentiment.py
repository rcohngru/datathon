...
import datetime
import pickle
# note: only need this if vader_lexicon has not been downloaded
...
def vader_analysis():
    ...
    Returns two dataframes of political tweets split by candidate with
    VADER scores.
    '''
    # load the data
    ...
    (data['pos'], data['neu'],
     data['neg'], data['com']) = score_data(data['clean_text'])

    # find candidates mentioned in each tweet
    data['biden'] = [True if 'biden' in tweet.lower() else False
                     for tweet in data['full_text']]
    data['trump'] = [True if 'trump' in tweet.lower() else False
                     for tweet in data['full_text']]

    # split tweets based on candidate mentions.
    # Note: there will be overlap amongst tweets that mention both
    ...
    biden_tweets = biden_tweets[~((biden_tweets['neu'] == 1) &
                                  (biden_tweets['com'] == 0))]
    trump_tweets = trump_tweets[~((trump_tweets['neu'] == 1) &
                                  (trump_tweets['com'] == 0))]
...
def plot_VADER():
    '''
        ...
        Input:
        -biden_avg: list of average vader sentiments on 3hr interval
        -trump_avg: list of average vader sentiments on 3hr interval
    ...
    # specify intervals
    intervals = pd.date_range(start='2020-09-30', end='2020-10-03',
                              freq='1H', tz='UTC')
    ...
    for i in range(len(intervals)):
        ...
        biden_set = biden_tweets[
                            (biden_tweets['created_at'] > intervals[start]) &
                            (biden_tweets['created_at'] < intervals[end])]

        trump_set = trump_tweets[
                            (trump_tweets['created_at'] > intervals[start]) &
    ...
    ax.text(trump_tweet + datetime.timedelta(hours=2), 0.25,
            'Trump Tweet', fontsize=13)
    ax.text(hicks_tweet - datetime.timedelta(hours=16), 0.15,
            'Hope Hicks Tweet', fontsize=13)
